...
wss.on('connection',ws=>{
  ...
  ws.on('message',buf=>{let m;try{m=JSON.parse(String(buf))}catch{return}
    ...
    if(m.type==='state'){
      ...
      else broadcastRoom(c.mode,{type:'state',player:{id:c.user.id,nickname:c.user.nickname,animal:c.user.animal||'lion',title:c.user.title||'나그네',x:c.x,y:c.y,dir:c.dir,frame:c.frame,moving:c.moving,mode:c.mode}},ws);
    } else if(m.type==='chat'){
