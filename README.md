...
## v28 NPC update
- 카페지기 NPC를 승인한 1번 기본형 사자 바리스타 디자인으로 교체했습니다.
- NPC 상호작용/카페 꾸미기/문 근처 E 나가기 동작은 그대로 유지됩니다.

## v29 — 외부 PNG NPC 교체 시스템
카페지기 NPC는 이제 코드로 그리지 않고 `images/npc/cafe-owner.png` 파일을 불러옵니다.
NPC 외형을 바꾸려면 새 투명 PNG의 파일명을 `cafe-owner.png`로 맞춘 뒤 같은 위치에 덮어쓰기만 하면 됩니다.
권장: 투명 배경, 세로형 캐릭터 PNG. 현재 게임 표시 크기는 72×90px이며 원본 PNG 해상도는 더 커도 됩니다.
